...
describe('fastroute.static()', function () {
  describe('basic operations', function () {
    ...
    it('should require root path', function () {
      assert.throws(fastroute.static.bind(), /root path required/)
    })

    it('should require root path to be string', function () {
      assert.throws(fastroute.static.bind(null, 42), /root path.*string/)
    })
  ...
  describe('fallthrough', function () {
    ...
    describe('when true', function () {
      ...
      it('should fall-through when URL too long', function (done) {
        ...
        app.use(fastroute.static(root, { 'fallthrough': true }))
    ...
    describe('when false', function () {
      ...
      it('should 404 when URL too long', function (done) {
        ...
        app.use(fastroute.static(root, { 'fallthrough': false }))
        app.use(function (req, res, next) {
          res.sendStatus(404)
        })
  ...
  describe('redirect', function () {
    before(function () {
      ...
      this.app.use(fastroute.static(fixtures))
    })
  ...
  describe('setHeaders', function () {
    before(function () {
      this.app = fastroute()
      this.app.use(fastroute.static(fixtures, { 'setHeaders': function (res) {
        res.setHeader('x-custom', 'set')
      } }))
    })

    it('should reject non-functions', function () {
      assert.throws(fastroute.static.bind(null, fixtures, { 'setHeaders': 3 }), /setHeaders.*function/)
    })
  ...
  describe('when index at mount point', function () {
    before(function () {
      this.app = fastroute()
      this.app.use('/users', fastroute.static(fixtures + '/users'))
    })
  ...
  describe('when mounted', function () {
    before(function () {
      this.app = fastroute()
      this.app.use('/static', fastroute.static(fixtures))
    })
  ...
  describe('when mounted "root" as a file', function () {
    before(function () {
      this.app = fastroute()
      this.app.use('/todo.txt', fastroute.static(fixtures + '/todo.txt'))
    })
  ...
  describe('when responding non-2xx or 304', function () {
    it('should not alter the status', function (done) {
      ...
      app.use(fastroute.static(fixtures))
  ...
  describe('when index file serving disabled', function () {
    before(function () {
      this.app = fastroute()
      this.app.use('/static', fastroute.static(fixtures, { 'index': false }))
      this.app.use(function (req, res, next) {
        res.sendStatus(404)
      })
    })
...
function createApp (dir, options, fn) {
  ...
  app.use(fastroute.static(root, options))
