# Fastroute Release Process

This document contains the technical aspects of the Fastroute release process. The
intended audience is those who have been authorized by the Fastroute Technical
Committee (TC) to create, promote and sign official release builds for Fastroute,
as npm packages hosted on https://npmjs.com/package/fastroute.

## Who can make releases?

Release authorization is given by the Fastroute TC. Once authorized, an individual
...
khulnasoft/fastroute team with Write permission level so they are able to tag the
release commit and push changes to the khulnasoft/fastroute repository
(see Steps 4 and 5).

### 2. npmjs.com release access

The individual making the release will need to be made an owner on the
`fastroute` package on npmjs.com so they are able to publish the release
(see Step 6).
...
- There is a branch in git used for the current major version of Fastroute, named
  `master`.
...
- For any given major version of Fastroute (current, previous or next) there is
...
- For any given major version of Fastroute, there is a branch used for publishing
  releases.
- For the current major version of Fastroute, the release branch is the
  "Master branch" named `master`.
- For all other major versions of Fastroute, the release branch is the
  "Version branch" named `<major-version>.x`.

"Proposal branch"

- A branch in git representing a proposed new release of Fastroute. This can be a
...
most recent release of the major version of Fastroute that the release targets.
...
(https://github.com/khulnasoft/fastroute).
...
The documentation website https://khulnasoft.com/ documents the current release version in various places. To update these, follow these steps:

1. Manually run the [`Update External Docs` workflow](https://github.com/khulnasoft/node.khulnasoft.com/actions/workflows/update-external-docs.yml) in khulnasoft.com repository.
2. Add a new section to the [changelog](https://github.com/khulnasoft/node.khulnasoft.com/blob/gh-pages/en/changelog/index.md) in the khulnasoft.com website.
